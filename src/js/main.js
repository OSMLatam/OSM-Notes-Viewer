...
async function searchUsers(query) {
  ...
  const results = index.filter(
    (user) =>
      user.username.toLowerCase().includes(query.toLowerCase()) || user.user_id.toString() === query
  );
...
function displaySearchResults(results, type) {
  ...
  animationManager.animateSearchResults(
    searchResults,
    results.map((item) => {
      if (type === 'user') {
        return {
  ...
  try {
    ...
    if (sortBy === 'closed') {
      sortedUsers = users.sort(
        (a, b) => (b.history_whole_closed || 0) - (a.history_whole_closed || 0)
      );
    } else {
...
async function loadTopCountries(page = 1, sortBy = 'open') {
  ...
  try {
    ...
    if (sortBy === 'closed') {
      ...
      sortedCountries = countries.sort(
        (a, b) => (b.history_whole_open || 0) - (a.history_whole_open || 0)
      );
    }
...
async function loadRecentNotesGlobal() {
  ...
  try {
    ...
    if (openArray.length > 0) {
      openContainer.innerHTML = openArray
        .map((noteId) => createSimpleNoteCard(noteId, 'open'))
        .join('');
    } else {
      openContainer.innerHTML =
        '<p style="text-align: center; color: var(--text-light); font-size: 0.9rem; padding: 1rem;">No recent open notes available</p>';
    }

    // Display closed notes (limit to 5)
    const closedArray = Array.from(closedNoteIds).slice(0, 5);
    if (closedArray.length > 0) {
      closedContainer.innerHTML = closedArray
        .map((noteId) => createSimpleNoteCard(noteId, 'closed'))
        .join('');
    } else {
      closedContainer.innerHTML =
        '<p style="text-align: center; color: var(--text-light); font-size: 0.9rem; padding: 1rem;">No recent closed notes available</p>';
    }
  } catch (error) {
    console.error('Error loading recent notes:', error);
    openContainer.innerHTML =
      '<p style="text-align: center; color: var(--text-light);">Error loading notes</p>';
    closedContainer.innerHTML =
      '<p style="text-align: center; color: var(--text-light);">Error loading notes</p>';
  }
}
...
async function loadActivityTimeline() {
  ...
  try {
    ...
    countries.forEach((country) => {
      ...
      country.dates_most_open.forEach((entry) => {
        ...
        if (date >= lastYear) {
          ...
          monthlyData.opened[monthKey] =
            (monthlyData.opened[monthKey] || 0) + (entry.quantity || 0);
        }
      });
      ...
      if (country.dates_most_closed) {
        country.dates_most_closed.forEach((entry) => {
          ...
          if (date >= lastYear) {
            const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
            monthlyData.closed[monthKey] =
              (monthlyData.closed[monthKey] || 0) + (entry.quantity || 0);
          }
        });
      }
    });
    ...
    const allMonths = new Set([
      ...Object.keys(monthlyData.opened),
      ...Object.keys(monthlyData.closed),
    ]);
    ...
    if (canvas && canvas.parentElement) {
      canvas.parentElement.innerHTML =
        '<p style="text-align: center; color: var(--text-light); padding: 2rem;">Unable to load activity data</p>';
    }
  }
}
