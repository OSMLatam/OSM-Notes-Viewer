...
function setupBaseLayerForMap(mapInstance, mapKey) {
  ...
  const currentBaseLayer =
    selectedBaseLayer === 'satellite' ? baseLayers.satellite : baseLayers.osm;
  ...
  if (mapKey === 'closedNotes') {
    ...
    setTimeout(() => {
      ...
      const correctBaseLayer =
        selectedBaseLayer2 === 'satellite' ? baseLayers.satellite : baseLayers.osm;
    ...
    if (mapKey === 'openNotes') {
      ...
      setTimeout(() => {
        ...
        const correctBaseLayer =
          selectedBaseLayer3 === 'satellite' ? baseLayers.satellite : baseLayers.osm;
  ...
  setTimeout(() => {
    ...
    const currentBaseLayer =
      selectedBaseLayerInner === 'satellite' ? baseLayers.satellite : baseLayers.osm;
    const otherBaseLayer =
      selectedBaseLayerInner === 'satellite' ? baseLayers.osm : baseLayers.satellite;
    ...
    setTimeout(() => {
      ...
      const correctBaseLayer =
        selectedBaseLayer === 'satellite' ? baseLayers.satellite : baseLayers.osm;
      const otherBaseLayer =
        selectedBaseLayer === 'satellite' ? baseLayers.osm : baseLayers.satellite;
      ...
      if (
        mapKey === 'closedNotes' &&
        (!wmsLayers.closedNotes || !wmsLayers.closedNotes.closedNotes)
      ) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            const selectedBaseLayerBeforeWMS = baseLayerSelectBeforeWMS
              ? baseLayerSelectBeforeWMS.value
              : 'osm';
            const baseLayerBeforeWMS =
              selectedBaseLayerBeforeWMS === 'satellite' ? baseLayers.satellite : baseLayers.osm;
            ...
            const selectedBaseLayerFinal = baseLayerSelectFinal
              ? baseLayerSelectFinal.value
              : 'osm';
            const finalBaseLayer =
              selectedBaseLayerFinal === 'satellite' ? baseLayers.satellite : baseLayers.osm;
    ...
    setTimeout(() => {
      ...
      if (mapKey === 'boundaries') {
        ...
        setTimeout(() => {
          ...
          activeMapInstance.setView(
            [currentCenter.lat + 0.000001, currentCenter.lng],
            currentZoom,
            {
              reset: false,
              animate: false,
...
async function initializeOpenNotesMap() {
  ...
  openNotesAttribution.addAttribution(
    '© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
  );

  // Set initial view to default location
  initialViews.openNotes = {
    center: [DEFAULT_LOCATION.lat, DEFAULT_LOCATION.lon],
    zoom: DEFAULT_ZOOM,
  };
...
async function initializeClosedNotesMap() {
  ...
  // Add OSM attribution manually to ensure it's always shown
  closedNotesAttribution.addAttribution(
    '© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
  );

  // Set initial view to default location
  initialViews.closedNotes = {
    center: [DEFAULT_LOCATION.lat, DEFAULT_LOCATION.lon],
    zoom: DEFAULT_ZOOM,
  };
...
async function initializeBoundariesMap() {
  ...
  // Add OSM attribution manually to ensure it's always shown
  boundariesAttribution.addAttribution(
    '© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
  );
...
function addWMSLayer(map, layerName, layerKey, bbox = null) {
  ...
  if (usePlugin && typeof L.WMS !== 'undefined' && typeof L.WMS.source !== 'undefined') {
    ...
    try {
      ...
      if (wmsLayer) {
        ...
        console.warn(
          `Could not create WMS layer: ${layerName}, falling back to native implementation`
        );
        throw new Error('Layer creation failed');
      }
    } catch (error) {
      console.warn(
        'Error using leaflet.wms plugin:',
        error,
  ...
  if (!wmsLayers[mapKey][layerKey] || !map.hasLayer(wmsLayers[mapKey][layerKey])) {
    ...
    const wmsLayer = L.tileLayer.wms(WMS_BASE_URL, {
      ...
      uppercase: true,
      attribution:
        '<a href="https://github.com/OSM-Notes/OSM-Notes-Analytics">OSM Notes Analytics</a>',
      tiled: true,
    ...
    if (!map._wmsClickHandlerAdded) {
      map.on('click', async (e) => {
        ...
        if (map === maps.openNotes && wmsLayers.openNotes && wmsLayers.openNotes.openNotes) {
          ...
          map === maps.closedNotes &&
          wmsLayers.closedNotes &&
          wmsLayers.closedNotes.closedNotes
        ) {
  ...
  try {
    ...
    if (e.latlng && e.info) {
      ...
      if (info.features && info.features.length > 0) {
        ...
        noteId =
          feature.properties?.note_id ||
          feature.properties?.id ||
          feature.properties?.noteId ||
          feature.id;
      } else if (info.properties) {
        noteId = info.properties.note_id || info.properties.id || info.properties.noteId;
      }
...
function toggleBoundariesLayer(layerKey, visible) {
  ...
  const wmsLayer =
    wmsLayers.boundaries && wmsLayers.boundaries[layerKey] ? wmsLayers.boundaries[layerKey] : null;
  ...
  try {
    ...
    if (permalinkBtn) {
      ...
      permalinkBtn.innerHTML =
        '<span aria-hidden="true">✓</span> <span data-i18n="map.controls.copied">Copied!</span>';
