...
function renderNoteInfo() {
  ...
  if (noteData.dateCreated) {
    ...
    document.getElementById('noteCreatedDate').textContent =
      `${i18n.t('note.action.created')}: ${formatDate(createdDate)}`;
  }
  ...
  if (noteData.dateClosed) {
    ...
    document.getElementById('noteUpdatedDate').textContent =
      `${i18n.t('note.action.closed')}: ${formatDate(closedDate)}`;
  ...
  // Comments count
  const commentsText =
    noteData.comments.length === 1 ? i18n.t('common.comment') : i18n.t('common.comments');
  document.getElementById('noteCommentsCount').textContent =
    `${noteData.comments.length} ${commentsText}`;
...
function renderNoteStats() {
  ...
  if (noteData.dateClosed && noteData.comments && noteData.comments.length > 0) {
    ...
    if (
      closedComment &&
      closedComment.user &&
      closedComment.user.user &&
      closedComment.user.user !== 'Anonymous'
    ) {
      const userLink = document.createElement('a');
...
function renderMap() {
  ...
  marker
    .bindPopup(
      `<b>${i18n.t('common.note')} #${noteData.id}</b><br>${i18n.t('hashtag.filters.status')} ${statusText}`
    )
    .openPopup();
  ...
  setTimeout(() => {
    try {
      ...
      worldMap.whenReady(() => {
        ...
        // Add marker
        const worldMarker = L.marker([noteData.lat, noteData.lon], { icon: smallPinIcon }).addTo(
          worldMap
        );
        ...
        worldMap.fitBounds(
          [
            [noteData.lat - 10, noteData.lon - 10],
            [noteData.lat + 10, noteData.lon + 10],
          ],
          {
            padding: [20, 20],
...
function renderInteractions() {
  ...
  const interactions = noteData.comments
    .map((comment, index) => {
      ...
      if (index === 0) {
        ...
        (noteData.status === 'closed' && index === noteData.comments.length - 1 && !comment.text)
      ) {
        actionType = 'closed';
...
function showHashtagSuggestions(text) {
  ...
  suggestionsContainer.innerHTML = availableHashtags
    .map((tag) => `<span class="hashtag-suggestion" data-hashtag="${tag}">${tag}</span>`)
    .join('');
...
async function handleComment() {
  ...
  if (!isAuthenticated()) {
    if (
      confirm(
        i18n.t('note.auth.loginRequired') ||
          'You need to log in with OpenStreetMap to comment on notes. Would you like to log in now?'
      )
    ) {
  ...
  if (!hasPermission('write_notes')) {
    alert(
      i18n.t('note.auth.insufficientPermissions') ||
        'Your account does not have permission to comment on notes.'
    );
    return;
  }
  ...
  try {
    ...
    if (error.message.includes('Authentication expired')) {
      errorMessage =
        i18n.t('note.auth.expired') || 'Your session has expired. Please log in again.';
      logout();
      updateAuthUI();
...
async function handleClose() {
  ...
  if (!isAuthenticated()) {
    if (
      confirm(
        i18n.t('note.auth.loginRequired') ||
          'You need to log in with OpenStreetMap to close notes. Would you like to log in now?'
      )
    ) {
      sessionStorage.setItem('oauth_return_url', window.location.href);
      initiateLogin();
  ...
  if (!hasPermission('write_notes')) {
    alert(
      i18n.t('note.auth.insufficientPermissions') ||
        'Your account does not have permission to close notes.'
    );
    return;
  }
  ...
  const closeWithComment =
    ...
    confirm(
      i18n.t('note.close.confirmWithComment') ||
        `Close note #${noteData.id}${commentText ? ' with your comment' : ''}?`
    );

  if (
    !closeWithComment &&
    !confirm(i18n.t('note.close.confirm') || `Are you sure you want to close note #${noteData.id}?`)
  ) {
    return;
  }
...
async function handleReopen() {
  ...
  if (!isAuthenticated()) {
    if (
      confirm(
        i18n.t('note.auth.loginRequired') ||
          'You need to log in with OpenStreetMap to reopen notes. Would you like to log in now?'
      )
    ) {
      sessionStorage.setItem('oauth_return_url', window.location.href);
      initiateLogin();
  ...
  if (!hasPermission('write_notes')) {
    alert(
      i18n.t('note.auth.insufficientPermissions') ||
        'Your account does not have permission to reopen notes.'
    );
    return;
  }

  const commentText = document.getElementById('commentText').value.trim();
  const reopenWithComment =
    commentText.length > 0 &&
    confirm(
      i18n.t('note.reopen.confirmWithComment') ||
        `Reopen note #${noteData.id}${commentText ? ' with your comment' : ''}?`
    );

  if (
    !reopenWithComment &&
    !confirm(
      i18n.t('note.reopen.confirm') || `Are you sure you want to reopen note #${noteData.id}?`
    )
  ) {
    return;
  }
...
async function handleReport() {
  ...
  if (
    confirm(
      i18n.t('note.report.confirm') ||
        `Report note #${noteData.id}? This will open OpenStreetMap's reporting page.`
    )
  ) {
...
async function loadMLRecommendation(noteId) {
  ...
  try {
    ...
    if (
      ...
      recommendation.action.toLowerCase() === 'map' &&
...
function generateBasicRecommendation(noteData) {
  ...
  if (hasClosingKeywords) {
    return {
      ...
      confidence: 0.6,
      reason:
        i18n.t('note.mlRecommendation.basicReason.close') ||
    ...
    if (hasKeyword) {
      ...
      if (category === 'building') {
        ...
        if (allText.includes('avenida') || allText.includes('avenue')) {
          ...
          allText.includes('camino') ||
          allText.includes('path') ||
          allText.includes('sendero')
        ) {
  ...
  if (hasMappingIntent && Object.keys(suggestedTags).length > 0) {
    return {
      ...
      tags: suggestedTags,
      reason:
        i18n.t('note.mlRecommendation.basicReason.map') ||
  ...
  if (hasCommentingKeywords) {
    return {
      ...
      confidence: 0.4,
      reason:
        i18n.t('note.mlRecommendation.basicReason.comment') ||
  ...
  if (noteData.text && noteData.text.trim().length > 10) {
    return {
      ...
      reason:
        i18n.t('note.mlRecommendation.basicReason.mapGeneric') ||
  ...
  if (action.toLowerCase() === 'map') {
    if (tags && Object.keys(tags).length > 0) {
      ...
      if (josmTagsInput) {
        ...
        josmTagsInput.placeholder =
          i18n.t('note.mlRecommendation.tagsPlaceholder') ||
          'Add OSM tags here (e.g., amenity=restaurant, name=...)';
      }
  ...
  try {
    ...
    if (!response.ok) {
      const getResponse = await fetch(
        `https://taginfo.openstreetmap.org/api/4/tag/values?key=${encodedKey}&value=${encodedValue}`,
        {
...
async function renderTagLinks(tagPairs, container) {
  ...
  const tagsToShow = tagPairs.filter(
    (t) =>
      t.key && t.value && !['name', 'note', 'source', 'created_by'].includes(t.key.toLowerCase())
  );
