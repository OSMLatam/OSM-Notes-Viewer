...
**Reference**: This strategy is aligned with [API_Proposal.md](../OSM-Notes-API/docs/API_Proposal.md) from the OSM-Notes-API project.
...
- ✅ **Natural progression**: User-Agent sufficient for most cases; OAuth only if specific functionality is needed
...
These endpoints **DO require OAuth** only if specific functionalities that need OSM user identity are implemented:
...
**Note**: Most endpoints do not require OAuth. User-Agent is sufficient for tracking and access control.
...
**Requirement**: All requests to the REST API MUST include a valid User-Agent with the application name.
...
async function checkRateLimit(req, endpointType = 'general') {
    ...
    if (current > limits.maxRequests) {
        throw new RateLimitError(`Rate limit exceeded: ${limits.maxRequests} requests per ${limits.windowMs / 1000 / 60} minutes`);
    }
...
class ClientRateLimiter {
    ...
    incrementCount(endpoint, endpointType) {
        const key = `${endpointType}:${endpoint}`;
        const limit = this.limits[endpointType];
        const record = this.requestCounts.get(key) || { count: 0, resetTime: Date.now() + limit.windowMs };
...
class AbuseDetector {
    ...
    isSuspiciousPattern() {
        ...
        const uniqueEndpoints = new Set(
            this.requestHistory.slice(-20).map(r => r.endpoint)
        );
...
- **[API_Proposal.md](../OSM-Notes-API/docs/API_Proposal.md)**: Aligned authentication model (User-Agent required, OAuth optional)
...
- **Note**: HDYC requires OAuth because it needs user personal data. In our case, most endpoints don't need specific user identity.
